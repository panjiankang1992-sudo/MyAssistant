...
class LocalSyncDatasource {
  ...
  Future<void> insertChangeRecord(
    String dataId,
    String dataType,
    String operation,
    String content,
  ...
  Future<List<ChangeRecord>> getPendingChanges() async {
    return (_db.select(
      _db.changeRecords,
    )..where((t) => t.pushed.equals(false))).get();
  }

  Future<void> markPushed(int recordId) async {
    await (_db.update(_db.changeRecords)
          ..where((t) => t.recordId.equals(recordId)))
        .write(const ChangeRecordsCompanion(pushed: Value(true)));
  }

  Future<void> markAllPendingPushed() async {
    await (_db.update(_db.changeRecords)..where((t) => t.pushed.equals(false)))
        .write(const ChangeRecordsCompanion(pushed: Value(true)));
  }

  Future<void> markPendingPushedForTypes(Set<String> dataTypes) async {
    if (dataTypes.isEmpty) return;
    await (_db.update(_db.changeRecords)
          ..where((t) => t.pushed.equals(false) & t.dataType.isIn(dataTypes)))
        .write(const ChangeRecordsCompanion(pushed: Value(true)));
  }

  Future<Map<String, int>> getPendingCountsByType() async {
    final rows = await getPendingChanges();
    final result = <String, int>{};
    for (final row in rows) {
      result[row.dataType] = (result[row.dataType] ?? 0) + 1;
    }
    return result;
  }
  ...
  Future<void> deletePushedRecords() async {
    await (_db.delete(
      _db.changeRecords,
    )..where((t) => t.pushed.equals(true))).go();
  }
